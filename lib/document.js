...
export class Document extends BaseDocument {
    ...
    static SCHEMA = () => ({_id: DatabaseClient.getClientInstance().nativeIdType()});
    ...
    async save() {
        ...
        if (schema[SCHEMA_REF_1_DOC_KEYS].length) {
            for (const key of schema[SCHEMA_REF_1_DOC_KEYS]) {
                if (docToSave[key]?.[IS_DOCUMENT]) {
                    docToSave[key] = DatabaseClient.toCanonicalId(docToSave[key]._id);
                }
            }
        }
        ...
        if (schema[SCHEMA_REF_N_DOCS_KEYS].length) {
            for (const key of schema[SCHEMA_REF_N_DOCS_KEYS]) {
                ...
                if (isArray(val) && val.length) {
                    docToSave[key] = val.map(v => v && v[IS_DOCUMENT] ? DatabaseClient.toCanonicalId(v._id) : v);
                }
            }
        }
        ...
        let newId = await DatabaseClient.getClientInstance().save(this.collectionName(), this._id, docToSave);
    ...
    async delete() {
        ...
        let totalDeleted = await DatabaseClient.getClientInstance().delete(this.collectionName(), this._id);
    ...
    static deleteOne(query) {
        return DatabaseClient.getClientInstance().deleteOne(this.collectionName(), query);
    }
    ...
    static deleteMany(query) {
        ...
        return DatabaseClient.getClientInstance().deleteMany(this.collectionName(), query);
    }
    ...
    static async findOne(query, options) {
        let data = await DatabaseClient.getClientInstance().findOne(this.collectionName(), query);
        if (!data) {
            return null;
        }
    ...
    static async findOneAndUpdate(query, values, options = {}) {
        ...
        let data = await DatabaseClient.getClientInstance().findOneAndUpdate(this.collectionName(), query, values, options);
        if (!data) {
            return null;
        }
    ...
    static findOneAndDelete(query, options) {
        ...
        return DatabaseClient.getClientInstance().findOneAndDelete(this.collectionName(), query, options);
    }
    ...
    static async find(query, options = {}) {
        ...
        let data = await DatabaseClient.getClientInstance().find(this.collectionName(), query, options);
     ...
     */
    static count(query) {
        return DatabaseClient.getClientInstance().count(this.collectionName(), query);
    }
    ...
    _createIndexesOnce() {
        for (const schemaEntry of this._schema[SCHEMA_ALL_ENTRIES]) {
            if (schemaEntry.unique || schemaEntry.indexed) {
                ...
                DatabaseClient.getClientInstance().createIndex(this.constructor.collectionName(), schemaEntry._key, {unique: !!schemaEntry.unique});
            }
        }
    }
    ...
    static clearCollection() {
        return DatabaseClient.getClientInstance().clearCollection(this.collectionName());
    }
